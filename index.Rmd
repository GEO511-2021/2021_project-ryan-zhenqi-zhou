...
I collected data from my own previous research projects. Most of them are downloaded from Baidu Map API (Baidu Map API, 2020).
...
Noted that the orange points are park entrances, green points are residential building POIs, blue lines are walking routes, red line is park boundary, and grey polygons are community boundaries.

## Method
The park’s pedestrian shed was delineated by walking routes presumably reached within 15 min. By virtue of these walking routes, I developed 6 indices which related to the walking routes to the park. The former covers two indices of the service capacity: service POIs and service area, while the latter covers four indices of the walking experience: route distance, pedestrian route directness (PRD), the number of turns, and the number of crossings (Table 2).
...
## Serivce areas at park level {.tabset}
A reasonable walking distance is necessary for daily park users. There are many measurement to define service areas of the park such as Euclidean distance buffer method, line-based network 50m-buffer method, and community boundary method.
...
As you can see in these three methods, the service areas determined by the line-based method and community boundary are significantly less than Euclidean distance buffer method. The line-based network buffer method and community boundary method are more accurate, as they are closer to the actual environment available to pedestrians. 
...
The service POIs across different entrances varied significantly, due to the extensive interface of Xuanwu Lake Park with various parts of the city (Figure 5). Entrances 8, 9, 26, and 1, which are located to the west and north of Xuanwu Lake Park, served more POIs: 217, 141, 123, and 118 POIs, respectively. Entrances 25, 24, and 22, located to the north of the park, served fewer POIs: 24, 15, and 9 POIs, respectively. Entrance 11 served the least POIs, just 6.

## Walking experience of the Routes from Communities to the Park {.tabset}
Taking the community boundary as the analysis unit, I analyze the walking experience from communities to the park. The pedestrian environment of route analysis covered the criteria of route distance, PRD, the number of turns, and the number of crossings.
